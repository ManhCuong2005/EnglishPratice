...
import { normalizeAnswer, REVIEW_RATINGS, shuffle, speakEnglish, speakText } from '../lib/learning.js';
...
const modes = [
  ...
  { id: 'meaning-listen', label: 'Nghe & viết nghĩa', icon: Headphones, description: 'Nghe từ rồi tự viết nghĩa theo hướng chọn.' },
...
function MeaningListening({ words, onReview, onFinish, onEnrichEnglishMeanings, setId, settings }) {
  ...
  const promptText = englishToVietnamese ? 'Nhập nghĩa tiếng Việt' : englishToEnglish ? 'Nhập từ hoặc nghĩa tiếng Anh' : 'Nhập từ tiếng Anh';
  ...
  const expectedAnswer = englishToVietnamese ? word.meaning : word.term;
  const expectedDisplay = englishToEnglish
    ? [word.term, word.englishMeaning].filter(Boolean).join(' · ')
    : expectedAnswer;
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      if (englishToVietnamese) speakEnglish(spokenText); else speakText(spokenText, spokenLanguage);
    }, 180);
    return () => window.clearTimeout(timer);
  }, [englishToVietnamese, index, settings.soundEnabled, spokenLanguage, spokenText, word]);
  ...
  const check = async (autoAdvance = false) => {
    ...
    const isCorrect = englishToVietnamese
      ...
      : englishToEnglish
        ? normalizeAnswer(answer) === normalizeAnswer(word.term) || matchesMeaning(answer, word.englishMeaning, true)
  ...
  return (
    <div className="meaning-listen-mode">
      <div className="meaning-listen-direction" role="group" aria-label="Chọn hướng luyện nghe">
        ...
        <button className={englishToEnglish ? 'active' : ''} type="button" onClick={() => changeDirection('en-to-en')}><Languages size={17} /><span><strong>Nghe tiếng Anh</strong><small>Viết từ / nghĩa tiếng Anh</small></span></button>
      ...
      <section className="meaning-listen-card">
        ...
        <div className={`meaning-listen-input ${checked === true ? 'correct' : checked === false || checked === 'skipped' ? 'wrong' : ''}`}><input ref={inputRef} autoFocus={!missingEnglishMeaning} value={answer} disabled={checked !== null} onChange={(event) => setAnswer(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') checked === null ? check(true) : next(); }} placeholder={englishToVietnamese ? 'Ví dụ: từ bỏ, ruồng bỏ' : englishToEnglish ? 'Ví dụ: unlucky hoặc not fortunate' : 'Ví dụ: abandon'} autoComplete="off" spellCheck="false" />{checked === true && <Check />}{(checked === false || checked === 'skipped') && <X />}</div>
        {(checked === false || checked === 'skipped') && <p className="meaning-listen-correct-answer">Đáp án đúng: <strong>{expectedDisplay || 'Chưa có đáp án'}</strong><button type="button" onClick={playPrompt}><Volume2 size={17} /></button></p>}
        {checked === true && <p className="meaning-listen-pass"><CheckCircle2 size={17} /> Chính xác — từ này đã được tính là pass.</p>}
        <div className="meaning-listen-actions">{checked === null && <Button variant="ghost" onClick={skip}>Bỏ qua</Button>}<Button onClick={checked === null ? check : next} disabled={checked === null && !answer.trim()}>{checked === null ? 'Kiểm tra' : index === questions.length - 1 ? 'Xem kết quả' : 'Tiếp theo'}</Button></div>
      </section>
    </div>
  );
}
